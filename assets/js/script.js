const greetings = ["Halo", "Hello", "Bonjour", "你好", "こんにちは", "안녕하세요"];
const helloWord = document.querySelector("#helloWord");
const navbar = document.querySelector(".navbar");
const themeToggle = document.querySelector("#themeToggle");
const themeMark = document.querySelector("#themeMark");
const songStart = document.querySelector("#songStart");
const songAudio = document.querySelector("#songAudio");
const songProgressFill = document.querySelector("#songProgressFill");
...
function rotateGreeting() {
  helloWord.classList.add("switching");

  window.setTimeout(() => {
    greetingIndex = (greetingIndex + 1) % greetings.length;
    helloWord.textContent = greetings[greetingIndex];
    helloWord.classList.remove("switching");
  }, 300);
}
...
function updateSongProgress() {
  ...
  const progressPercent = duration ? (songAudio.currentTime / duration) * 100 : 0;
  songProgressFill.style.width = `${Math.min(progressPercent, 100)}%`;
  songCurrentTime.textContent = formatSongTime(songAudio.currentTime);
  songDurationText.textContent = formatSongTime(duration);
}

function pauseFavoriteSong() {
  songAudio.pause();
  favoritePlayer.classList.remove("is-playing");
  songStart.setAttribute("aria-label", "Play song");
}

function playFavoriteSong() {
  favoritePlayer.classList.add("is-playing");
  songStart.setAttribute("aria-label", "Pause song");
...
if (songStart && songAudio && songProgressFill && songCurrentTime && songDurationText && favoritePlayer) {
  updateSongProgress();
  ...
  songAudio.addEventListener("loadedmetadata", updateSongProgress);
  songAudio.addEventListener("timeupdate", updateSongProgress);
